...
const Login = () => {
  ...
  const signin=()=>{
    signInWithEmailAndPassword(auth,usernameSin,passwordSin).then(
      (response)=>{setRes("SuccesFully Logined");document.cookie=`_hash=${response.user.uid}`;setNav(1); navigate("/")}
